...
class SPPE_FastPose(object):
    def __init__(self,
        ...
        if backbone == 'resnet101':
            self.model = InferenNet_fast().to(device)   # 见mainfast_inference.py
        else:
            self.model = InferenNet_fastRes50().to(device)
        self.model.eval()

    def predict(self, image, bboxs, bboxs_scores):
        inps, pt1, pt2 = crop_dets(image, bboxs, self.inp_h, self.inp_w)    # 见img.py 502行; pt1[i]和pt2[i]是每个方框的左上和右下点
        ...
        xy_hm, xy_img, scores = getPrediction(pose_hm, pt1, pt2, self.inp_h, self.inp_w,
                                              pose_hm.shape[-2], pose_hm.shape[-1])     # eval.py 113行 获取关键点位置
